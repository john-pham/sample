...
@Component({
    selector: 'paymentvouchers',
    templateUrl: './paymentvouchers.component.html',
    styleUrls: ['./paymentvouchers.component.css'],
...
export class PaymentVouchersComponent implements OnInit, OnDestroy {
    ...
    private getIO() {
        //load user
        this.ioservice.getiopayment(this.filterName, this.filterValue, 0, true, this.ioId, this.fromDate, this.toDate, 0, 0).subscribe(results => this.onDataLoadSuccessfulPayment(results), error => this.onDataLoadFailed(error));
            ...
            this.route.paramMap
                .switchMap((params: ParamMap) => {
                    ...
                    if (ioid != null && ioid.length > 0) {
                        this.ioservice.getiopayment(this.filterName, this.filterValue, 0, true, ioid, this.fromDate, this.toDate, 0, 0).subscribe(resulted => {
    ...
    private onDataLoadSuccessfulDefault(io: Payment) {
        this.pointer = io;
    }

    private getPaymentTypes() {
        //load user
        this.localService.getPaymentTypes(true).subscribe(results => this.onDataLoadSuccessfulStudents(results), error => this.onDataLoadFailed(error));
    }
