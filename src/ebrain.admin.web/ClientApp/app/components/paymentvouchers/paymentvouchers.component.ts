...
export class PaymentVouchersComponent implements OnInit, OnDestroy {
    ...
    private getIO() {
        //load user
        this.ioservice.getiopayment(this.filterName, this.filterValue, true, this.ioId, this.fromDate, this.toDate).subscribe(results => this.onDataLoadSuccessfulPayment(results), error => this.onDataLoadFailed(error));
            ...
            this.route.paramMap
                .switchMap((params: ParamMap) => {
                    ...
                    if (ioid != null && ioid.length > 0) {
                        this.ioservice.getiopayment(this.filterName, this.filterValue, true, ioid, this.fromDate, this.toDate).subscribe(results => {
                            results.forEach(row => {
                                this.onMappingIOToPaymentDetail(row);
                            });
