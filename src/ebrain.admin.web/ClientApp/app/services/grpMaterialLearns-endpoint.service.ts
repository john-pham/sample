...
@Injectable()
export class GrpMaterialLearnsEndpoint extends EndpointFactory {

    private readonly _serviceUrl: string = "/api/grpMaterialLearns";
    ...
    findFromTypeId(typeid: string): Observable<Response> {
        let url = this.getUrl('findbytypeid?typeid=' + typeid + '');
        return this.http.get(url, this.getAuthHeader())
            .map((response: Response) => {
                return response;
            })
            .catch(error => {
                return this.handleError(error, () => this.findFromTypeId(typeid));
            });
    }
