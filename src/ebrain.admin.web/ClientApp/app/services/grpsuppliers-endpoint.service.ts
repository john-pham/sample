...
@Injectable()
export class GrpsuppliersEndpoint extends EndpointFactory {

    private readonly _serviceUrl: string = "/api/grpsuppliers";
    ...
    getAll(option: number): Observable<Response> {

        let url = this.getUrl('getall?option=' + option);
        ...
        return this.http.get(url, this.getAuthHeader())
            ...
            .catch(error => {
                return this.handleError(error, () => this.getAll(option));
            });
    }
