...
@Injectable()
export class MaterialLearnsEndpoint extends EndpointFactory {
    ...
    getMaterialLearn(filter: string, value: string, page: number, size: number): Observable<Response> {

        if (typeof value == "undefined") {
            value = "";
        }
        let url = this.getUrl('getallmateriallearnlist?filter=' + filter + '&value=' + value + '&page=' + page + '&size=' + size + '&hash_id=' + Math.random());
        ...
        return this.http.get(url, this.getAuthHeader())
            ...
            .catch(error => {
                return this.handleError(error, () => this.getMaterialLearn(filter, value, page, size));
            });
    }
