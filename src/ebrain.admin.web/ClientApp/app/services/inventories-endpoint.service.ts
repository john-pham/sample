...
@Injectable()
export class InventoriesEndpoint extends EndpointFactory {

    private readonly _serviceUrl: string = "/api/inventories";
    ...
    updateInventories(filter: string, value: string, fromDate: Date, toDate: Date): Observable<Response> {

        let url = this.getUrl('updatedinventories?filter=' + filter + '&value=' + value + '&fromDate=' + fromDate + '&toDate=' + toDate + '&hash_id=' + Math.random());
