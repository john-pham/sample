...
namespace ebrain.admin.bc.Repositories
{
    public class InventoriesRepository : Repository<Inventory>, IInventoriesRepository
    {
        ...
        public Task<IEnumerable<Inventory>> GetAlls()
        {
            throw new NotImplementedException();
        }

        public Task<IEnumerable<InventoryDetail>> GetDetailByIOId(Guid? id)
        {
            throw new NotImplementedException();
        }
        ...
        public IEnumerable<InventorieslList> GetInventoryList(DateTime fromDate, DateTime toDate, string filter, string branchIds)
        {
            try
            {
                ...
                List<InventorieslList> someTypeList = new List<InventorieslList>();
                this.appContext.LoadStoredProc("dbo.sp_Inventory_BranchIds")
        ...
        public async Task<bool> UpdateInventory(DateTime fromDate, DateTime toDate, string filter, string branchIds, Guid userId)
        {
            try
            {
                ...
                else
                {
                    ...
                    for (var i = 0; i <= months; i++)
                    {
                        ...
                        if (results != null && results.Count() > 0)
                        {
                            ...
                            //get inventory
                            var inv = this.appContext.Inventory.FirstOrDefault(
                            ...
                            var invds = this.appContext.InventoryDetail.Where(p => p.InventoryId == inv.InventoryId);
                            foreach (var item in results)
                            {
                                var itemExist = invds.FirstOrDefault(p => p.Materialid == item.MaterialId);
                                if (itemExist != null)
                                {
                                    itemExist.InputQuantity = item.QuantityEnd;
                                    itemExist.UpdatedBy = userId;
                                    itemExist.UpdatedDate = DateTime.Now;
                                }
                                else
                                {
                                    itemExist = new InventoryDetail
                                    {
                                        InventoryDetailId = Guid.NewGuid(),
                                        InventoryId = inventoryId,
                                        InputQuantity = item.QuantityEnd,
                                        CreatedBy = userId,
        ...
        public IEnumerable<Inventory> GetTopActive(int count)
        {
            throw new NotImplementedException();
        }

        public Task<Inventory> Save(Inventory value, InventoryDetail[] iosd, Guid? id)
        {
            throw new NotImplementedException();
        }

        public Task<IEnumerable<Inventory>> Search(string filter, string value)
        {
            throw new NotImplementedException();
        }
    }
